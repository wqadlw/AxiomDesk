...
USER_KEYWORDS = {
    "朋友推荐": 1, "群里": 1, "老师": 1, "带我": 1, "内幕": 2, "稳赚": 2,
    "必涨": 1, "翻倍": 1, "稳赚不赔": 2, "包赚": 2,
}

def trap_detect(features: dict, keyword_boost: int = 0) -> dict:
    roe = features.get("roe", 10); nm = features.get("net_margin", 10)
    sent = features.get("sentiment", 5); mom = features.get("momentum", 0)
    ...
    return {
        "signals": signals, "hits": base_hits, "weighted_hits": weighted,
        "trap_score": trap_score, "trap_level": level,
...
def great_divide(results: list[dict], features: dict, val: dict, trap: dict) -> dict:
    ...
    bull_arg = {
        "估值": f"综合公允价相对现价约 {upside:+.0%}，安全边际尚可" if upside >= 0 else "现价已低于内在价值，越跌越便宜",
        "成长": f"营收增速 {features.get('revenue_growth',0):.0f}%、护城河 {features.get('moat',0):.1f}/10，趋势向上" if mom > 0 else "行业地位与护城河仍在，只是暂时逆风",
        "风险": f"陷阱评级 {trap['trap_level']}，无显著杀猪盘特征",
    }
    bear_arg = {
        "估值": f"综合公允价相对现价约 {upside:+.0%}，并不便宜" if upside < 0 else "估值已隐含乐观预期，上行空间有限",
        "成长": f"净利率 {features.get('net_margin',0):.0f}%、负债率 {features.get('debt_ratio',0)*100:.0f}%，质量存疑" if features.get("debt_ratio",0) > 0.6 else "增速能否持续是最大未知数",
        "风险": f"陷阱评级 {trap['trap_level']}，{'高度疑似杀猪盘，必须回避' if trap['trap_score'] <= 3 else '短期动量过热，追高需谨慎'}",
    }

    rounds = [
        {"topic": "估值与安全边际", "bull": f"{bull['name']}：{bull_arg['估值']}。", "bear": f"{bear['name']}：{bear_arg['估值']}。"},
        {"topic": "成长与护城河", "bull": f"{bull['name']}：{bull_arg['成长']}。", "bear": f"{bear['name']}：{bear_arg['成长']}。"},
        {"topic": "风险与时机", "bull": f"{bull['name']}：{bull_arg['风险']}。", "bear": f"{bear['name']}：{bear_arg['风险']}。"},
    ]

    punch = (f"多方代表 {bull['name']}（{bull['group_name']}）与空方代表 {bear['name']}（{bear['group_name']}）"
...
def analyze(ticker: str, keyword_boost: int = 0, depth: str = "deep", use_ai: bool = True) -> dict:
    ...
    features = DP.derive_features(profile)
    ...
    meta = {
        "ticker": ticker, "name": profile["name"], "market": profile["market"],
        "industry": profile["industry"], "source": profile["source"],
        "unit": profile.get("unit", "RMB亿"),
        "price": profile["price"], "mcap": profile["mcap_yi"], "mcap_unit": profile.get("unit", "亿"),
        "pe": profile["pe"], "pb": profile["pb"], "ps": profile["ps"],
        "roe": profile["roe"], "revenue_growth": profile["rev_growth"],
        "debt_ratio": profile["debt_ratio"], "moat": profile["moat"],
        "momentum": profile["momentum"], "volatility": profile["volatility"],
        "shares_yi": profile["shares_yi"], "revenue_yi": profile["revenue_yi"],
        "net_margin": profile["net_margin"], "fcf_yi": profile.get("fcf_yi"),
        "ebitda_yi": profile.get("ebitda_yi"), "total_debt_yi": profile.get("total_debt_yi"),
        "cash_yi": profile.get("cash_yi"), "equity_yi": profile.get("equity_yi"),
        "eps": profile.get("eps"), "bvps": profile.get("bvps"),
        "beta": profile.get("beta"), "institutional_ratio": profile.get("instr_ratio"),
        "sentiment": profile.get("sentiment"), "lhb_count": profile.get("lhb_count"),
        "is_small_cap": features.get("is_small_cap"), "is_large_cap": features.get("is_large_cap"),
        "is_financial": features.get("is_financial"), "is_tech": features.get("is_tech"),
        "ai_theme": features.get("ai_theme"), "is_liquor": features.get("is_liquor"),
        "is_new_energy": features.get("is_new_energy"), "is_cyclical": features.get("is_cyclical"),
        "is_hot_theme": features.get("is_hot_theme"), "trend_up": features.get("trend_up"),
        "is_oversold": features.get("is_oversold"), "is_accelerating": features.get("is_accelerating"),
