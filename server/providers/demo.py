...
def _synthetic(ticker: str) -> dict:
    ...
    return dict(
        ...
        unit=unit,
        price=px,
        shares_yi=round(shares, 2),
        mcap_yi=round(mcap, 1),
        revenue_yi=rev,
        net_margin=nm,
        fcf_yi=round(rev * nm / 100 * r.uniform(0.5, 0.9), 1),
        ebitda_yi=round(rev * nm / 100 / 0.6, 1),
        total_debt_yi=round(mcap * debt_ratio, 1),
        cash_yi=round(mcap * r.uniform(0.05, 0.4), 1),
        equity_yi=round(mcap * (1 - debt_ratio * 0.5), 1),
        eps=round(px / pe, 3),
        bvps=round(px / pb, 2),
        pe=pe,
        pb=pb,
        ...
        moat=moat,
        momentum=round(r.uniform(-0.25, 0.30), 3),
        volatility=round(r.uniform(0.18, 0.65), 2),
        beta=round(r.uniform(0.4, 2.2), 2),
        instr_ratio=round(r.uniform(10, 70), 1),
        sentiment=round(r.uniform(3, 8), 1),
        lhb_count=r.randint(0, 6),
        source="合成演示数据(非真实行情)",
    )
...
class DemoDataProvider(DataProvider):
    ...
    def get_peers(self, ticker: str, p: dict, n: int = 5) -> list[dict]:
        ...
        while len(peers) < n:
            ...
            peers.append(
                {
                    "name": f"{p.get('industry', '同业')}可比#{len(peers) + 1}(合成)",
                    "ticker": None,
