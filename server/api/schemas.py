...
class AnalyzeParams(BaseModel):
    ...
    depth: Literal["lite", "medium", "deep"] = Field(
        "deep", description="lite=快速(10评委) / medium=标准(51) / deep=深度(66+多空辩论)"
    )
    use_ai: bool = Field(
        True,
        description="是否调用大模型补全研判叙述（维度评语/多空辩论/核心结论/风险/买入区间）。关闭则走离线模板，响应更快。",
    )
